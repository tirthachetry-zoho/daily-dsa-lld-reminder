...
export async function POST(request: Request) {
  try {
    ...
    const emailHtml = `
      ...
      <body style="font-family: Arial, sans-serif; line-height: 1.6; color: #333; max-width: 600px; margin: 0 auto; padding: 20px;">
        ...
        <div style="background: #eff6ff; border-radius: 8px; padding: 20px; margin-bottom: 20px;">
          ...
          <h3 style="font-size: 24px; color: #1e3a8a; margin: 15px 0;">${dsaProblem?.title || "Two Sum"}</h3>
          
          <div style="margin: 15px 0;">
            <span style="background: #dcfce7; color: #166534; padding: 5px 15px; border-radius: 20px; font-size: 14px; font-weight: bold;">
              ${dsaProblem?.difficulty || "EASY"}
            </span>
            <span style="color: #666; margin-left: 10px;">${dsaProblem?.topic || "Arrays"}</span>
          </div>
          ...
          <div style="margin: 15px 0;">
            ...
            ${dsaProblem.companies.map(c => `<span style="background: #f3f4f6; padding: 4px 10px; border-radius: 4px; font-size: 12px; margin-right: 5px; display: inline-block;">${c}</span>`).join('')}
          </div>
          ` : ''}
        ...
        <div style="background: #faf5ff; border-radius: 8px; padding: 20px; margin-bottom: 20px;">
          ...
          <h3 style="font-size: 24px; color: #5b21b6; margin: 15px 0;">${systemDesignProblem?.title || "Design a URL Shortener"}</h3>
    ...
    await resend.emails.send({
      ...
      to: email,
      subject: "🚀 Daily Coding Reminder - Preview",
      html: emailHtml,
    });

    return NextResponse.json({ message: "Preview email sent successfully" });
  } catch (error) {
